            ...
            <table className="items-center w-full bg-transparent border-collapse">
              <thead>
                <tr>
                  ...
                  <th className="px-6 bg-gray-50 text-gray-500 align-middle border border-solid border-gray-100 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left dark:bg-zinc-700 dark:text-gray-300 dark:border-zinc-600 cursor-pointer" onClick={() => handleSort('status_name')}>Status {sortField === 'status_name' && (sortOrder === 'asc' ? '▲' : '▼')}</th>
                  <th className="px-6 bg-gray-50 text-gray-500 align-middle border border-solid border-gray-100 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left dark:bg-zinc-700 dark:text-gray-300 dark:border-zinc-600 cursor-pointer" onClick={() => handleSort('priority_name')}>Prioridade {sortField === 'priority_name' && (sortOrder === 'asc' ? '▲' : '▼')}</th>
                  ...
                  <tr key={ticket.id} className="dark:bg-zinc-800">
                    ...
                    <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 dark:border-zinc-700">
                      <span className={`px-2 py-1 rounded-full text-xs font-semibold
                        ${ticket.status_name === 'Aberto' ? 'bg-red-200 text-red-800 dark:bg-red-700 dark:text-red-100' :
                          ticket.status_name === 'Em Andamento' ? 'bg-yellow-200 text-yellow-800 dark:bg-yellow-700 dark:text-yellow-100' :
                          ticket.status_name === 'Aguardando Cliente' ? 'bg-orange-200 text-orange-800 dark:bg-orange-700 dark:text-orange-100' :
                          ticket.status_name === 'Resolvido' ? 'bg-green-200 text-green-800 dark:bg-green-700 dark:text-green-100' :
                    ...
                    <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 dark:border-zinc-700">
                      <span className={`px-2 py-1 rounded-full text-xs font-semibold
                        ${ticket.priority_name === 'Urgente' ? 'bg-red-200 text-red-800 dark:bg-red-700 dark:text-red-100' :
                          ticket.priority_name === 'Alta' ? 'bg-orange-200 text-orange-800 dark:bg-orange-700 dark:text-orange-100' :
                          ticket.priority_name === 'Média' ? 'bg-yellow-200 text-yellow-800 dark:bg-yellow-700 dark:text-yellow-100' :
